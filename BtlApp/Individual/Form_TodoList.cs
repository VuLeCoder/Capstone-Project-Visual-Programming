...
namespace BtlApp.Individual
{
    public partial class Form_TodoList : Form
    {
        ...
        private int userId; // Lưu trữ UserId của người dùng hiện tại
        private int editingId = 0; // Lưu trữ ToDoId của task đang chỉnh sửa, 0 nếu là thêm mới
        private readonly DataProcesser Db = new DataProcesser();
        public Form_TodoList(Form_Manager manger , int curentUserId)
        {
            this.manager = manger;
        ...
        private void dgvTodo_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvTodo.Columns[e.ColumnIndex].Name == "Delete")
            {
                ...
                {
                    string query = "DELETE FROM tbl_ToDoList WHERE ToDoId = @id";
        ...
        {
            string query = "INSERT INTO tbl_ToDoList (UserId, Title, Description, Priority, Status, CreatedAt, DueDate) " +
        ...
        {
            string query = "UPDATE tbl_ToDoList SET Title=@title, Description=@desc, Priority=@prio, Status=@status, DueDate=@due WHERE ToDoId=@id";
            var prms = new SqlParameter[]
            {
                new SqlParameter("@title", txtTitle.Text.Trim()),
                new SqlParameter("@desc", txtDescription.Text.Trim()),
        ...
        {
            string query = "SELECT * FROM tbl_ToDoList WHERE ToDoId = @id AND UserId = @userId";
            var prms = new SqlParameter[]
            {
            new SqlParameter("@id", id),
